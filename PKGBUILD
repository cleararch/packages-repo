...
pkgver=2023.01.31
pkgrel=1
pkgdesc='Base ClearArch files(for livecd)'
arch=('x86_64')
license=('GPL')
url='https://cleararch.github.io'
depends=('iana-etc')
backup=('etc/crypttab' 'etc/fstab' 'etc/group' 'etc/gshadow' 'etc/host.conf'
        'etc/hosts' 'etc/issue' 'etc/ld.so.conf' 'etc/nsswitch.conf'
        'etc/passwd' 'etc/profile' 'etc/resolv.conf' 'etc/securetty'
        'etc/shadow' 'etc/shells' 'etc/subuid' 'etc/subgid')
source=('cleararch-logo.png' 'crypttab' 'env-generator' 'fstab' 'group' 'gshadow' 'host.conf' 'hosts'
...
package(){
  ...
  install -D -m644 "$srcdir"/sysctl usr/lib/sysctl.d/10-ca.conf

  # setup systemd-sysusers
  install -D -m644 "$srcdir"/sysusers usr/lib/sysusers.d/ca.conf

  # setup systemd-tmpfiles
  install -D -m644 "$srcdir"/tmpfiles usr/lib/tmpfiles.d/ca.conf

  # setup systemd.environment-generator
  install -D -m755 "$srcdir"/env-generator usr/lib/systemd/system-environment-generators/10-ca

  # add logo
  install -D -m644 "$srcdir"/cleararch-logo.png usr/share/pixmaps
}
